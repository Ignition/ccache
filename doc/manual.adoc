...
*modules*::
    ...
    *Use case*: Builds using `-fmodules`, or Clang builds using
...
The `-fmodules` option selects "`Clang modules`" and, on GCC, standard C++20
named modules. Both have these requirements:

* Set <<config_sloppiness,*sloppiness*>> to `modules`.
* Enable both <<The direct mode,*direct mode*>> and <<The depend mode,*depend
  mode*>>.
* Write dependency information, for example with `-MD`. This is what tells
  ccache which module files the compilation reads. A compilation that writes
  none is counted as "`Could not use modules`".
...
For GCC named modules:

* GCC names the binary module interfaces that an import resolves to in the
  dependency information, and ccache hashes them, so a consumer is recompiled
  when a module it imports changes.
* Compiling a module interface also writes the interface itself, which ccache
  does not store. Such a compilation is never served from the cache, so the
  module stays available to consumers, and only compilations that import a
  module benefit.
* GCC records a timestamp in the interface, so recompiling one yields different
  bytes for the same source. A consumer hits only while the interfaces it
  imports are left in place, as in an incremental build; one that rebuilds them
  gets a miss.
